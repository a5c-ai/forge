...
process.stdin.on("end", () => {
  ...
  const evidenceId = signalId ? `${signalId}_report` : "command_report";

  const cmd = String(input?.producer_args?.cmd || "").trim();
  const artifactRoot = String(input?.artifact_root || "").trim();
  if (!cmd) {
    process.stdout.write(JSON.stringify({ ok: true, evidence: [{ evidence_id: evidenceId, kind: "report", metrics: { failed: 1 }, summary: "missing cmd" }] }) + "\n");
    return;
  }
