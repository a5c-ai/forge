...
process.stdin.on("end", () => {
  ...
  const run = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        A5C_RUN_ID: runId,
        A5C_STEP_ID: String(stepId),
        A5C_ATTEMPT: String(attempt),
        A5C_AGENT_PROFILE: profile,
      },
    },
  );
  if (run.status !== 0) {
  ...
  // Validate that the agent wrote the required JSON footer.
  const outMd = fs.readFileSync(agentOutPath, "utf8");
  const footerMatch = /```json\s*\n([\s\S]*?)\n```\s*$/.exec(outMd.trim());
  if (!footerMatch) {
    process.stdout.write(JSON.stringify({ ok: false, error: "missing required json footer" }) + "\n");
    return;
  }
