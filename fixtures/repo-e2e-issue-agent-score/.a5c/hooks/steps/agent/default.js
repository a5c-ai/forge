...
process.stdin.on("end", () => {
  ...
  const issueId = String(process.env.A5C_TEST_ISSUE_ID || "");
  ...
  const agentOutPath = path.join(tmpDir, "agent-output.md");
  ...
  if (issueId) genArgs.push("--var", `issueId=${issueId}`);
  genArgs.push("--out", promptPath);

  const gen = spawnSync(a5cCli.cmd, genArgs, { encoding: "utf8" });
  if (gen.status !== 0) {
  ...
  const run = spawnSync(
    a5cCli.cmd,
    [...a5cCli.argv, "agent", "run", "--profile", profile, "--in", promptPath, "--out", agentOutPath],
    { encoding: "utf8" },
  );
  if (run.status !== 0) {
