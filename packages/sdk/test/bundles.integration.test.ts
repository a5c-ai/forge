...
import { openRepo, loadSnapshot, renderIssue } from "../src/index.js";
...
describe("NDJSON bundles (integration)", () => {
  it("loads events from .bundle.ndjson and renders them", async () => {
    const root = path.resolve(import.meta.dirname, "../../..");
    const fixture = path.join(root, "fixtures", "repo-bundles");

    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "a5cforge-bundles-"));
    ...
    const issue = renderIssue(snap, "issue-1");
    expect(issue).toBeTruthy();
    expect(issue!.comments.map((c) => c.commentId)).toEqual(["c1", "c2"]);
  });
});
