...
describe("CLI deps waiting + dep completion (orchestration MVP)", () => {
  it(
    "blocks parent until dependent run is DONE",
    async () => {
      const repo = await makeRepoFromFixture("repo-orchestration-deps-wait");

      expect(
        await runCli(["run", "dispatch", "--repo", repo, "--playbook", "playbooks/parent.yaml@v1", "--run-id", "run_001"], {
          stdout: () => {},
          stderr: () => {}
        })
      ).toBe(0);
      ...
      const parentEventsDir = path.join(repo, ".collab", "runs", "run_001", "events");
      ...
      expect(
        await runCli(["run", "reconcile", "--repo", repo, "--run-id", "run_001"], {
          stdout: () => {},
          stderr: () => {}
        })
      ).toBe(30);
