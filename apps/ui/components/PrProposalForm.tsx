...
export function PrProposalForm() {
  ...
  return (
      ...
      onSubmit={async (e) => {
        ...
        try {
          const res = await fetch(`/api/prs/${encodeURIComponent(prKey.trim())}/proposal`, {
            ...
            body: JSON.stringify({ baseRef: baseRef.trim(), headRef: headRef.trim(), title: title.trim(), body: body.trim() || undefined })
          });
    ...
    >
      <div className="text-sm font-medium text-zinc-200">New PR proposal</div>
      <div className="mt-2 grid gap-2 md:grid-cols-4">
        ...
        <div className="grid gap-1">
          <label className="text-xs text-zinc-400">Head ref</label>
          <input
            className="rounded border border-zinc-800 bg-zinc-950 px-2 py-1 text-sm text-zinc-100"
            value={headRef}
      ...
      <div className="mt-2 flex items-center gap-2">
        <button
          ...
          disabled={busy || !prKey.trim() || !baseRef.trim() || !headRef.trim() || !title.trim()}
          type="submit"
        >
