...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (cfg.remoteUrl) {
      ...
      const r = await fetch(`${cfg.remoteUrl.replace(/\/$/, "")}/v1/issues/${encodeURIComponent(id)}/claim${refParam}`, {
        method: "POST",
        headers: {
    ...
    const msg = String(body.commitMessage ?? `a5c: claim ${id} ${op} ${agentId}`);
    ...
    const writeIn = async (repoRoot: string) => {
      ...
      const wr = await writeAgentClaimChanged(
        ...
        { agentId, entity: { type: "issue", id }, op, note, time }
      );
