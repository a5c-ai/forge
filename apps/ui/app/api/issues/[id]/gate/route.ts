...
import { HlcClock, loadHlcState, openRepo, saveHlcState, writeGateChanged } from "@a5c-ai/sdk";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (cfg.remoteUrl) {
      ...
      const r = await fetch(`${cfg.remoteUrl.replace(/\/$/, "")}/v1/issues/${encodeURIComponent(id)}/gate${refParam}`, {
        method: "POST",
        headers: {
